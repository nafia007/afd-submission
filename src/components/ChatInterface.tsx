    ...
    try {
      ...
      const response = await fetch('https://api.hyperbolic.xyz/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [...chatHistory, newMessage], // Include full chat history
          max_tokens: 512,
          temperature: 0.1,
          top_p: 0.9,
          stream: false
    ...
    <Dialog>
      <DialogTrigger asChild>
        <Button 
          variant="outline" 
          size="icon"
          className="fixed right-4 bottom-4 h-12 w-12 rounded-full bg-accent hover:bg-accent/90"
        >
        ...
        <Card className="h-full bg-black/80 backdrop-blur-lg border-accent/20">
          <div className="flex flex-col h-full p-4">
            {!localStorage.getItem('DEEPSEEK_API_KEY') && (
              <form onSubmit={handleApiKeySubmit} className="flex gap-2 mb-4">
                <Input
                  type="password"
                  value={apiKey}
                  onChange={(e) => setApiKey(e.target.value)}
                  placeholder="Enter your DeepSeek API key..."
                  className="flex-1 bg-black/50 border-accent/20 text-white"
                />
            ...
            <form onSubmit={handleSubmit} className="flex gap-2 mt-4">
              <Input
                ...
                className="flex-1 bg-black/50 border-accent/20 text-white placeholder:text-white/50"
              />
              <Button 
                type="submit" 
                disabled={isLoading || !message.trim() || !apiKey}
                variant="outline"
                className="border-accent/20 text-white hover:bg-accent/20"
              >
                {isLoading ? (
